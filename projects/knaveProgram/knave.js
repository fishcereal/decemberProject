...
traits = [
  ...
  hair = ['Hair','Bald','Braided','Bristly'],
  clothing = ['Clothing','Antique','Bloody','Ceremonial'],
  virtue = ['Virtue','Ambitious','Cautious','Courageous'],
  vice = ['Vice','Aggressive','Arrogant','Bitter'],
  speech = ['Speech','Blunt','Booming','Breathy'],
  Background = ['Background','Alchemist','Beggar','Butcher'],
  misfortunes = ['Misfortune','Abandoned','Addicted','Blackmailed']
  ];
...
var setAllButtonNames = function(){
  $('#genderNewLine').text(''+ traitValues[0]);
  $('#physiqueNewLine').text(''+ traitValues[1]);
  $('#faceNewLine').text(''+ traitValues[2]);
  $('#skinNewLine').text(''  + traitValues[3]);
  $('#hairNewLine').text('' + traitValues[4]);
  $('#clothingNewLine').text(''+ traitValues[5]);
  $('#virtueNewLine').text(''+ traitValues[6]);
  $('#viceNewLine').text(''+ traitValues[7]);
  $('#speechNewLine').text(''+ traitValues[8]);
  $('#backgroundNewLine').text(''+ traitValues[9]);
  $('#misfortuneNewLine').text(''+ traitValues[10]);
}
...
setAllAbilityScoresRandom = function (){
  strengthBonus = findLeast(randomNumber(6,1),randomNumber(6,1),randomNumber(6,1));
  dexterityBonus = findLeast(randomNumber(6,1),randomNumber(6,1),randomNumber(6,1));
  constitutionBonus = findLeast(randomNumber(6,1),randomNumber(6,1),randomNumber(6,1));
  intelligenceBonus = findLeast(randomNumber(6,1),randomNumber(6,1),randomNumber(6,1));
  wisdomBonus = findLeast(randomNumber(6,1),randomNumber(6,1),randomNumber(6,1));
  charismaBonus = findLeast(randomNumber(6,1),randomNumber(6,1),randomNumber(6,1));
  ...
  hp = randomNumber(8,1);
  $('#strength').text('Strength: ' + strengthBonus + ' | ' + strengthDefense);
  $('#dexterity').text('Dexterity: ' + dexterityBonus + ' | ' + dexterityDefense);
  $('#constitution').text('Constitution: ' + constitutionBonus + ' | ' + constitutionDefense);
  $('#intelligence').text('Intelligence: ' + intelligenceBonus + ' | ' + intelligenceDefense);
  $('#wisdom').text('Wisdom: ' + wisdomBonus + ' | ' + wisdomDefense);
  $('#charisma').text('Charisma: ' + charismaBonus + ' | ' + charismaDefense);
...
setItemStatsRandom = function(){
  rollHelmetAndShield(); 
  rollArmor();
  $('#itemSlots').text('Starting item slots: ' + (constitutionDefense));
  ...
  $('#otherGear').text('In additional to your armor you carry 2 rations, ' + dungeoneeringGear[randomNumber(dungeoneeringGear.length-1,1)] + ', ' + dungeoneeringGear[randomNumber(dungeoneeringGear.length-1,1)] + ', ' + generalGear1[randomNumber(generalGear1.length-1,1)] + ', and ' + generalGear2[randomNumber(generalGear2.length-1,1)] + '. Each of these items takes up 1 slot unless the DM says otherwise')
